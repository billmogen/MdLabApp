...
//        String currentWifiName = wifiInfo.getSSID();
//        Log.d(TAG, "init: currentWifiName" + currentWifiName );
//        scanResults = wifiManager.getScanResults();
        ...
        if (scanResults == null) {
            ...
            while (it.hasNext()) {
                ...
                if (it.next().SSID.toString().startsWith("MDLab")) {
    ...
    public class DeviceRecyclerViewAdapter
        ...
        private final List<ScanResult> mListWifi;
        //private final WifiInfo mWifiInfo;
        public DeviceRecyclerViewAdapter(List<ScanResult> items) {
            mListWifi = items;
        }
        ...
        public void onBindViewHolder(final ViewHolder holder, int position) {
           // holder.mDeviceItem = mListWifi.get(position).SSID.toString();
            final ScanResult tScanResult = mListWifi.get(position);
            ...
            String [] tmp = null;
            tmp = tScanResult.SSID.toString().split("_");

            if (tmp[1].equals("WeatherStation")) {
                String str = String.format(getString(R.string.mdlab_device_weatherstation));
                str = str + tmp[2].toString();
                holder.mDeviceName.setText(str);
            }
//            holder.mDeviceName.setText(tScanResult.SSID.toString());
...
//                        fragment.setArguments(arguments);
                        DeviceDetailFragment fragment = DeviceDetailFragment.newInstance(tScanResult.SSID.toString());
                        ...
                        Log.d(TAG, "onClick: DeviceName " + tScanResult.SSID.toString());
                        intent.putExtra(DeviceDetailFragment.ARG_PARAM1, tScanResult.SSID.toString());
